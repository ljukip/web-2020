    ...
    <h1>Welcome <span style="color: seashell;">{{user.username}}</span></h1>
        <div id="Div-panel">
            <div style="flex-direction: row;">
                    ...
                    <button v-if="user.role ==='GUEST'" class="oval" style="background-image: url('images/pp.jpg');display: inline-block;"@click="$router.push('/') "><p> </p></button>
            </div>
            <div style="flex-direction: row;">
                <router-link style="width: 40%;" to='/'>apartments</router-link>
                <router-link style="width: 20%;" to='/'>reservations</router-link>
                <router-link v-if="user.role ==='GUEST'" style="width: 38%;" to='/'>profile</router-link>
